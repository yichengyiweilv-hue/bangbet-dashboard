 ...
 * - 次留/七留：多选
 *
 * 需要 index.html 提供这些容器 id（如果没有，本脚本会自动在 card-paid-retention 区块内创建）：
 * - retention-view / retention-months / retention-windows / retention-countries / retention-medias / retention-productTypes
 * - chart-paid-retention / table-title-retention / table-retention
 */

(function () {
  const MODULE_KEY = "retention";
  ...
  function ensureNonEmpty(state, opts) {
    ...
    if (!state.windows.length) state.windows = ["D1", "D7"];
  }
  ...
  function formatPct01(ratio, digits) {
    if (ratio === null || ratio === undefined || !isFinite(Number(ratio)))
      return "-";
  ...
  // 复用 roi 模块注入的通用样式（同一个 STYLE_ID，避免重复注入）
  ...
  function createFilterPanelFallback(sectionEl) {
    const exists =
      getDom("retention-view") ||
      getDom("retention-months") ||
      getDom("retention-countries") ||
      getDom("retention-medias") ||
      getDom("retention-productTypes") ||
      getDom("retention-windows") ||
      getDom("retention-btn-reset");

    if (exists) return;

    const panel = document.createElement("div");
    panel.className = "chart-filter-panel";
    panel.id = "retention-filter-panel";
    panel.innerHTML = `
      <div class="hero-filter-row">
        <div class="label">视图</div>
        <div class="chart-mini-radio" id="retention-view"></div>
      </div>
      ...
      <div class="hero-filter-row">
        <div class="label">留存</div>
        <div class="chart-mini-chips" id="retention-windows"></div>
      </div>

      <div style="display:flex;justify-content:flex-end;gap:8px;">
        <button type="button" class="btn" id="retention-btn-reset">全选 / 重置</button>
      </div>
    `;

    const header = sectionEl ? sectionEl.querySelector(".chart-card-header") : null;
  ...
  function createTableFallbackAfter(chartEl) {
    ...
    section.id = "table-section-retention";
    section.innerHTML = `
      <div class="chart-table-title" id="table-title-retention"></div>
  ...
  function init() {
    ...
    const chartEl = getDom("chart-paid-retention") || getDom("chart-retention");
    if (!chartEl) return;
    if (!window.echarts) return;

    const sectionEl =
      chartEl.closest(".chart-card") ||
      getDom("card-paid-retention") ||
      chartEl.parentNode;
    ...
    // 表容器（没有就自动创建）
    let tableTitleEl = getDom("table-title-retention");
    let tableEl = getDom("table-retention");
    if (!tableEl) {
      createTableFallbackAfter(chartEl);
      tableTitleEl = getDom("table-title-retention");
      tableEl = getDom("table-retention");
    }
    ...
    function buildTitleSuffix() {
      ...
      const mediaText = state.collapse.medias
        ...
        ? "全部媒体"
        : state.medias.map((v) => normalizeMedia(v)).join("+");
    ...
    function renderRadioView() {
      const container = getDom("retention-view");
      if (!container) return;
      container.innerHTML = "";
      ...
      const makeRadio = (label, value, checked) => {
        ...
        input.type = "radio";
        input.name = "retention-view-radio";
        input.value = value;
    ...
    function filteredRowsForMonth(monthKey, extra) {
      ...
      const mediaSet = state.collapse.medias
        ? null
        : new Set(uniq(state.medias.map(normalizeMedia)));
      ...
      return rows.filter((r) => {
        ...
        // 始终限制在 GH/KE/NG/TZ
        if (FIXED_COUNTRY_ORDER.indexOf(c) === -1) return false;
    ...
    function renderBar() {
      ...
      let countriesAxis = sortCountries(
        uniq(state.countries.map(normalizeCountry))
      ).filter((c) => FIXED_COUNTRY_ORDER.indexOf(c) !== -1);
      ...
      monthsSel.forEach((monthKey, mi) => {
        ...
        windowsSel.forEach((win) => {
          const retainedField =
            win === "D7" ? "D7_retained_users" : "D1_retained_users";
          const seriesName = `${mLabel}${winLabel(win)}`;
          ...
          const data = countriesAxis.map((countryKey) => {
            ...
            const sums = sumFields(rows, ["registration", retainedField]);
            return safeDiv(sums[retainedField], sums.registration);
          });
    ...
    function renderLine() {
      ...
      monthsSel.forEach((monthKey) => {
        (RAW[monthKey] || []).forEach((r) => {
          ...
          // 始终限制在 GH/KE/NG/TZ
          if (FIXED_COUNTRY_ORDER.indexOf(c0) === -1) return;
          ...
          if (!byBase[baseKey][d])
            byBase[baseKey][d] = { registration: 0, d1: 0, d7: 0 };

          const bucket = byBase[baseKey][d];

          const reg = Number(r.registration);
          if (isFinite(reg)) bucket.registration += reg;

          const d1v = Number(r.D1_retained_users);
          if (isFinite(d1v)) bucket.d1 += d1v;

          const d7v = Number(r.D7_retained_users);
      ...
      const baseKeysSorted = baseKeys.slice().sort((a, b) => {
        const A = parseBaseKey(a);
        const B = parseBaseKey(b);

        const ca = state.collapse.countries ? "ALL" : A.c;
        const cb = state.collapse.countries ? "ALL" : B.c;
        const ia = FIXED_COUNTRY_ORDER.indexOf(ca);
        const ib = FIXED_COUNTRY_ORDER.indexOf(cb);
        if (ia !== ib) return (ia === -1 ? 999 : ia) - (ib === -1 ? 999 : ib);

        const ma = state.collapse.medias ? "ALL" : A.m;
        const mb = state.collapse.medias ? "ALL" : B.m;
        if (ma !== mb) return String(ma).localeCompare(String(mb));

        const pa = state.collapse.productTypes ? "ALL" : A.p;
        const pb = state.collapse.productTypes ? "ALL" : B.p;
        return String(pa).localeCompare(String(pb));
      });

      function seriesNameFromBase(baseKey, win) {
        const parts = parseBaseKey(baseKey);

        const nameParts = [];
        if (!state.collapse.countries) nameParts.push(parts.c);
        if (!state.collapse.medias) nameParts.push(parts.m);
        if (!state.collapse.productTypes)
          nameParts.push(String(parts.p).toUpperCase());
        nameParts.push(winLabel(win));

        return nameParts.join(" · ");
      }

      const series = [];
      baseKeysSorted.forEach((baseKey, idx) => {
        ...
        windowsSel.forEach((win) => {
          const data = dates.map((d) => {
            ...
            const rv = win === "D7" ? bucket.d7 : bucket.d1;
            return safeDiv(rv, bucket.registration);
          });
    ...
    function renderTable() {
      ...
      let rowCountries = sortCountries(
        uniq(state.countries.map(normalizeCountry))
      ).filter((c) => FIXED_COUNTRY_ORDER.indexOf(c) !== -1);
      ...
      if (tableTitleEl)
        tableTitleEl.textContent = `当前筛选 · 次留/七留（%）${suffix}`;
      ...
      monthsSel.forEach((m) => {
        windowsSel.forEach((w) => {
          colKeys.push({ m, w });
          trh.appendChild(th(`${formatMonthLabel(m)}${winLabel(w)}`));
        });
      });
      ...
      function getCell(monthKey, win, countryKey, mediaKey, typeKey) {
        ...
        const sums = sumFields(rows, ["registration", retainedField]);
        const ratio = safeDiv(sums[retainedField], sums.registration);
    ...
    function renderAll() {
      ...
      renderChipGroup({
        containerId: "retention-countries",
      ...
      renderChipGroup({
        containerId: "retention-medias",
        values: opts.medias,
        stateArray: state.medias,
        max: 0,
        getLabel: (v) => normalizeMedia(v),
        allowEmpty: false,
        specialAllNoBreakdown: true,
        collapseKey: "medias",
      });
      ...
      renderChipGroup({
        containerId: "retention-productTypes",
      ...
      renderChipGroup({
        containerId: "retention-windows",
        values: ["D1", "D7"],
        stateArray: state.windows,
        max: 0,
        getLabel: (v) => winLabel(v),
        allowEmpty: false,
      });
    ...
    const resetBtn = getDom("retention-btn-reset");
    if (resetBtn) {
      resetBtn.addEventListener("click", () => {
        ...
        if (!state.months.length)
          setArray(state.months, (opts.months || []).slice(-1));

        setArray(state.countries, (opts.countries || []).slice());
        setArray(state.medias, (opts.medias || []).slice());
        setArray(state.productTypes, (opts.productTypes || []).slice());
        setArray(state.windows, ["D1", "D7"]);
